...
dre_methods = ['iterkul', 'itercpeexp', 'iterlog', 'itersquare']
...
for da_method in da_methods:
    for dre_method in dre_methods:
        for res_dir, extractor_dir in zip(res_dirs, extractor_dirs):
            for rcond in rconds:                
                et = EnsembleTrainer(base_dir=res_dir + f'/{da_method}', da_method=da_method, seed_list=seeds, extractor=extractor_dir,
                                     rcond=rcond, manual_filter_lambdas=manual_filter_lambdas, suffix=suffix, dre_method=dre_method, cv=True, agg=True)
                et.run()
...
for da_method in da_methods:
    for dre_method in dre_methods:
        for res_dir, extractor_dir in zip(res_dirs, extractor_dirs):
            for rcond in rconds:                
                et = EnsembleTrainer(base_dir=res_dir + f'/{da_method}', da_method=da_method, seed_list=seeds, extractor=extractor_dir,
                                     rcond=rcond, manual_filter_lambdas=manual_filter_lambdas, suffix=suffix, dre_method=dre_method, cv=True, agg=True, bma=True)
                et.run()                
...
for da_method in da_methods:
    for dre_method in dre_methods:
        for res_dir, extractor_dir in zip(res_dirs, extractor_dirs):
            for rcond in rconds:                
                et = EnsembleTrainer(base_dir=res_dir + f'/{da_method}', da_method=da_method, seed_list=seeds, extractor=extractor_dir,
                                     rcond=rcond, manual_filter_lambdas=manual_filter_lambdas, suffix=suffix, dre_method=dre_method, cv=True, agg=True, sup=True)
                et.run()                                                               
