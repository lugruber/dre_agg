...
from misc.dre_aggregation import dre_aggregation
...
class EnsembleTrainer():
    ...
    def __init__(self, base_dir: str, da_method: str, extractor: str,
                 ...
                 dre_method=None, cv=True, niter=1, adapt_reg=False, reg_par=None, agg=False, dre_rcond=1e-3, bma=False, sup=False):
    ...
    def run(self):
        ...
        for seed in self.seeds: #seeds_:
            ...
            for domain in self.ExtractorClass.domains: #domains_:
                ...
                iwv_dict = self.extractor.get_iwv_predictions(domain=domain, seed=seed, dre_method=self.dre_method,
                                                              ...
                                                              reg_par=self.reg_par, agg=self.agg)

                if self.agg:
                    param_grid = {'reg_par': list(10 ** (-np.arange(start=-8, step=1, stop=6, dtype=float)))}                    
                    hess_reg_par = param_grid['reg_par'][0]
                    iwv_dict = dre_aggregation(iwv_dict=iwv_dict, hess_reg_par=hess_reg_par, hess_mod_id=0,
                                               rcond=self.dre_rcond, bma=self.bma, sup=self.sup)
        ...
        # acc_file = str(save_dir / f'accuracies--eps-{options.eps}.npz') # TODO add epsilon to filename
        if self.cv:
